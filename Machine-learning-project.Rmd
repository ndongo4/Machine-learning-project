...
cfMat.EN
```
La matrice de confusion donne un bon taux de classement (78\%) sur le jeu test avec une bonne sensitivité  de 93\% et une spécificité assez faible de 47%.
...
y <-  cancerTraining %>% 
  mutate(Y=as.integer(if_else(Y=="no",0,1))) %>% 
  select(Y) %>% 
  as.matrix()
#y <- to_categorical(y.train)
...
model %>%
layer_dense(units = 20, activation = "softmax", input_shape = c(2)) 
